...
function MatchCard({ match }: { match: CricketMatch }) {
  ...
  // For upcoming matches, show start time from score field (we embed it there)
  const upcomingTime = isUpcoming ? match.team1.score : ''
    ...
    <div className={`bg-white rounded-xl shadow-lg border overflow-hidden transition-all hover:shadow-xl ${
      ...
      <div className={`px-5 py-3 flex items-center justify-between ${
        ...
        <div className="flex items-center gap-2">
          ...
          {isUpcoming && (
            <span className="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-0.5 rounded">UPCOMING</span>
          )}
        </div>
      </div>

      {/* Upcoming: show teams + start time */}
      {isUpcoming ? (
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-600">{match.team1.name}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-row-reverse">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-slate-600">{match.team2.name}</div>
              </div>
            </div>
          </div>
          {upcomingTime && upcomingTime !== 'Upcoming' && (
            <div className="flex items-center gap-1.5 text-xs text-blue-700 bg-blue-50 rounded-lg px-3 py-1.5">
              <Clock className="w-3.5 h-3.5" />
              <span className="font-medium">{upcomingTime}</span>
            </div>
          )}
        </div>
      ) : (
